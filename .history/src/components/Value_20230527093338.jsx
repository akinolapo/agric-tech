...
const Value = () => {
  return (
    <div className='mt-10 block md:grid md:gap-4 md: grid-cols-2 lg:grid-cols-3 py-10 px-5 sm:px-32'>
        {
            data.map(({id, icon, title, description})=>{
                return(
                    <div key={id} className='border-solid border-2 border-green-200 p-6 rounded-2xl mb-4 md:mb-0'>
            {icon}
            <h3>{title}</h3>
            <p>{description}</p>
        </div>
